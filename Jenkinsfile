...
//Déclencheur (webhook GitHub)
    ...
    stage{
        ...
        stage('Checkout') { //Récupère le dépôt GitHub, branche main
            steps {
        ...
        stage('Install dependencies - Backend'){
            steps{
                dir('back'){sh 'npm install'}
            }
        }

        stage('Install dependencies - Frontend'){
            steps{
                dir('front'){sh 'npm install'}
            }
        }
        ...
        stage('Build Docker Images') {
            steps {
                script {
                    ...
                    sh "docker build -t $DOCKER_HUB_USER/$BACK_IMAGE:latest ./back"
                }
            }
        }
        ...
        // Étape 10 : Smoke test pour vérifier que les services sont joignables
        stage('Smoke Test') {
            steps {
                sh '''
